...
st.set_page_config(page_title="Pricing Options & Swaps", layout="wide", page_icon="📈")

class BlackScholesOption:
    """Pricing et Greeks pour options européennes avec Black-Scholes"""
...
def get_live_data(ticker='AAPL', lookback_days=252):
    """Récupère données live et calcule volatilité historique"""
    try:
...
# Interface Streamlit
st.title("📈 Pricing d'Options Européennes & Interest Rate Swaps")

tabs = st.tabs(["🎯 Options avec données réelles", "💱 Interest Rate Swap", "⚙️ Options personnalisées"])

# TAB 1: Options avec données réelles
with tabs[0]:
    st.header("Options avec données de marché")
        ...
        if st.button("📊 Récupérer les données", type="primary"):
            with st.spinner("Chargement des données..."):
                ...
                if success:
                    ...
                    st.success(f"✓ Données récupérées pour {ticker}")
    ...
    if 'spot' in st.session_state:
        ...
        col2.metric("Volatilité historique", f"{hist_vol*100:.2f}%")
        ...
        with col1:
            ...
            maturity_call = st.slider("Maturité (mois)", 1, 24, 3, key="mat_call") / 12
            ...
            st.metric("💰 Prix du Call", f"${call.price():.4f}")
        ...
        with col2:
            ...
            maturity_put = st.slider("Maturité (mois)", 1, 24, 6, key="mat_put") / 12
            ...
            st.metric("💰 Prix du Put", f"${put.price():.4f}")
...
with tabs[1]:
    ...
    with col2:
        tenor = st.slider("Durée (années)", 1, 30, 5)
        payment_freq = st.selectbox("Fréquence de paiement", [1, 2, 4, 12], index=1, 
    ...
    for bp_change in [-50, -25, -10, 0, 10, 25, 50]:
        ...
        scenarios_data.append({
            'Scénario': f"{bp_change:+d} bp",
            'Taux': f"{new_rate*100:.3f}%",
            'Valeur': value,
            'Δ vs Base': value - swap.value(discount_rate)
        })
...
# TAB 3: Options personnalisées
with tabs[2]:
    st.header("Options avec paramètres personnalisés")
    ...
    with col1:
        ...
        custom_maturity = st.slider("Maturité (années)", 0.01, 5.0, 0.25, 0.01)
    
    with col2:
        custom_rate = st.slider("Taux sans risque (%)", 0.0, 20.0, 5.0, 0.1) / 100
        custom_vol = st.slider("Volatilité (%)", 1.0, 100.0, 25.0, 1.0) / 100
    ...
    col1, col2, col3 = st.columns(3)
    col1.metric("💰 Prix de l'option", f"${custom_option.price():.4f}")
    col2.metric("📊 Moneyness", f"{(custom_spot/custom_strike - 1)*100:+.2f}%")
    col3.metric("⏱️ Jours restants", f"{int(custom_maturity * 365)}")
    ...
    greeks_df = pd.DataFrame({
        ...
        'Interprétation': [
            'Sensibilité à +$1 du spot',
            'Sensibilité du Delta',
            'Sensibilité à +1% de vol',
            'Decay par jour',
            'Sensibilité à +1% de taux'
        ]
    })
